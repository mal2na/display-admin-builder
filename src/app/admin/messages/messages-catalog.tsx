  ...
  return (
    <div className="flex h-full flex-col p-6">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3 rounded-xl border bg-card p-2.5 text-[12px]">
        <span className="flex items-center gap-1.5 font-semibold"><LayoutGrid className="h-4 w-4 text-indigo-500" /> 후보 있는 코너 <b className="text-foreground">{cornersWith}</b></span>
        ...
        <div className="ml-auto flex items-center gap-2">
          <label className="flex cursor-pointer items-center gap-1.5 text-[11px] text-muted-foreground">
            <input type="checkbox" checked={onlyWith} onChange={(e) => setOnlyWith(e.target.checked)} className="accent-indigo-600" />
            후보 있는 코너만 <span className="text-slate-400">(끄면 전체 {corners.length})</span>
          </label>
          <div className="relative">
            <Search className="pointer-events-none absolute left-2 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" />
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="코너·문구·타겟 검색" className="h-8 w-48 rounded-lg border bg-white pl-7 pr-2 text-xs outline-none focus:ring-2 focus:ring-indigo-200" />
          </div>
        </div>
      </div>
                ...
                {cs.map((c) => {
                  ...
                  return (
                    <button key={c.cornerId} onClick={() => setSelId(c.cornerId)}
                      className={cn('flex w-full items-center gap-2 rounded-xl border p-2.5 text-left transition-colors', sel ? 'border-indigo-400 bg-indigo-50/50 ring-1 ring-indigo-200' : 'bg-card hover:border-slate-300')}>
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="mt-0.5 flex items-center gap-2 text-[10px]">
                          ...
                          {c.variantCount > 0 && <span className="text-indigo-600">후보 {c.variantCount}</span>}
                          {c.excludedCount > 0 && <span className="text-slate-400">제외 {c.excludedCount}</span>}
                        </span>
                      </span>
                      <ChevronRight className={cn('h-4 w-4 shrink-0', sel ? 'text-indigo-400' : 'text-slate-300')} />
                    </button>
                  );
                })}
              </div>
    ...
    <div className="flex h-full flex-col">
      <div className="flex items-start gap-3 border-b p-5">
        <div className="min-w-0 flex-1">
          <p className="flex items-center gap-2 text-[16px] font-bold text-foreground">{corner.cornerName} <span className="rounded bg-indigo-100 px-1.5 py-0.5 text-[11px] font-semibold text-indigo-700">{corner.cornerType}</span></p>
        ...
        {corner.slots.map((s) => (
          <section key={`${s.kind}:${s.holderId}`} className="rounded-xl border">
            <div className="flex items-center gap-2 border-b bg-slate-50/60 px-4 py-2.5">
              <span className={cn('grid h-6 w-6 place-items-center rounded-md', s.kind === 'title' ? 'bg-indigo-100 text-indigo-600' : 'bg-sky-100 text-sky-600')}>
...
function AddVariant({ slot, cornerId, library }: { slot: Slot; cornerId: string; library: LibEntry[] }) {
  ...
  if (!open) {
    return (
      <button type="button" onClick={() => setOpen(true)} className="mt-2 inline-flex items-center gap-1 rounded-md border border-indigo-300 bg-indigo-50 px-2.5 py-1 text-[11px] font-medium text-indigo-700 hover:bg-indigo-100">
  ...
  return (
    <div className="mt-2 rounded-lg border border-indigo-200 bg-indigo-50/40 p-2.5">
      <div className="flex items-center gap-1.5">
        ...
        <button type="button" onClick={() => setMode(mode === 'lib' ? null : 'lib')} className={cn('inline-flex h-8 shrink-0 items-center gap-1 rounded-md border px-2 text-[11px] font-medium', mode === 'lib' ? 'border-indigo-400 bg-white text-indigo-700' : 'bg-white text-slate-600 hover:bg-slate-50')} title="문구 라이브러리에서 불러오기"><Library className="h-3.5 w-3.5" /> 라이브러리</button>
        <button type="button" onClick={() => setMode(mode === 'ai' ? null : 'ai')} className={cn('inline-flex h-8 shrink-0 items-center gap-1 rounded-md border px-2 text-[11px] font-medium', mode === 'ai' ? 'border-indigo-400 bg-white text-indigo-700' : 'bg-white text-slate-600 hover:bg-slate-50')} title="AI 문구 제안(예시)"><Wand2 className="h-3.5 w-3.5" /> AI 제안</button>
      ...
      {mode === 'lib' && (
        <div className="mt-2 rounded-md border bg-white p-2">
          <div className="mb-1.5 flex items-center gap-1.5">
            ...
            <span className="text-[10px] font-semibold text-slate-500">문구 라이브러리 <span className="font-normal text-slate-400">· 같은 용도(<b className="text-indigo-600">{slot.use}</b>)만 재사용</span></span>
          ...
          <div className="max-h-44 space-y-0.5 overflow-y-auto">
            ...
            {libHits.map((e, i) => (
              <button key={i} type="button" onClick={() => setText(e.text)} className="flex w-full items-center gap-1.5 rounded px-2 py-1 text-left hover:bg-indigo-50">
      ...
      {mode === 'ai' && (
        <div className="mt-2 rounded-md border bg-white p-2">
          <p className="mb-1.5 flex items-center gap-1.5 text-[10px] font-semibold text-slate-500"><Wand2 className="h-3.5 w-3.5 text-indigo-500" /> AI 제안 <span className="font-normal text-slate-400">· {target || '타겟없음'} 기준 · 예시(실서비스는 AI 생성) · 클릭해 채택</span></p>
          {aiHits.length === 0 ? (
            <p className="py-2 text-center text-[11px] text-slate-400">기본 문구가 있어야 제안할 수 있어요</p>
          ) : (
            <div className="space-y-0.5">
              {aiHits.map((p, i) => (
                <button key={i} type="button" onClick={() => setText(p)} className="flex w-full items-center gap-1.5 rounded px-2 py-1 text-left text-[12px] text-slate-700 hover:bg-indigo-50">
                  <Sparkles className="h-3 w-3 shrink-0 text-indigo-400" /> <span className="truncate">{p}</span>
                </button>
              ))}
            </div>
          )}
